...
export function AdminGallery() {
  ...
  async function archiveMedia(id: string) {
    if (!window.confirm("Arquivar esta mídia?")) return;
    const response = await fetch(`/api/v1/admin/gallery/media/${id}`, { method: "DELETE", credentials: "include", headers: { "X-CSRF-Token": csrf } });
    if (!response.ok) return setError("Não foi possível arquivar a mídia.");
    setMedia((current) => current.filter((item) => item.id !== id)); setMessage("Mídia arquivada.");
  }

  async function archiveAlbum() {
    if (!currentAlbum || !window.confirm("Arquivar este álbum?")) return;
    const response = await fetch(`/api/v1/admin/gallery/albums/${currentAlbum.id}`, { method: "DELETE", credentials: "include", headers: { "X-CSRF-Token": csrf } });
    if (!response.ok) return setError("Não foi possível arquivar o álbum.");
    setAlbums((current) => current.filter((item) => item.id !== currentAlbum.id)); setAlbumId(""); setMedia([]); setMessage("Álbum arquivado.");
  }
  ...
  return <section className={`section ${styles.workspace}`}>
    ...
    <div className={styles.topGrid}>
      ...
      <div className={styles.panel}><h2>Álbum ativo</h2><label>Selecione o álbum<select value={albumId} onChange={(event) => setAlbumId(event.target.value)}><option value="">Selecione</option>{albums.map((item) => <option key={item.id} value={item.id}>{item.title} — {statusLabel(item.status)}</option>)}</select></label>{currentAlbum ? <><p className="muted">/{currentAlbum.slug}</p><button className="button secondary" type="button" disabled={!media.some((item) => item.status === "published")} onClick={() => void changeStatus("album", currentAlbum.id, currentAlbum.status === "published" ? "draft" : "published")}>{currentAlbum.status === "published" ? "RETORNAR A RASCUNHO" : "PUBLICAR ÁLBUM"}</button>{" "}<button className="button secondary" type="button" onClick={() => void archiveAlbum()}>ARQUIVAR ÁLBUM</button></> : <p className="muted">Crie ou selecione um álbum para começar.</p>}</div>
    </div>
    {albumId && <>
      ...
      <div className={styles.sectionTitle}><h2>Mídias do álbum</h2><span className="muted">{media.length} {media.length === 1 ? "item" : "itens"}</span></div><div className={styles.mediaGrid}>{media.map((item, index) => <article className="card" key={item.id}><h2>{item.title}</h2><p className="muted">{item.category} · {item.width ?? "?"}×{item.height ?? "?"}</p><p>{item.alt_text}</p><small>{statusLabel(item.status)}</small><p><button className="button secondary" type="button" disabled={index === 0} onClick={() => void moveMedia(index, -1)} aria-label={`Mover ${item.title} para cima`}>↑</button>{" "}<button className="button secondary" type="button" disabled={index === media.length - 1} onClick={() => void moveMedia(index, 1)} aria-label={`Mover ${item.title} para baixo`}>↓</button></p><p><button className="button secondary" type="button" onClick={() => void changeStatus("media", item.id, item.status === "published" ? "draft" : "published")}>{item.status === "published" ? "RETORNAR A RASCUNHO" : "PUBLICAR MÍDIA"}</button>{" "}<button className="button secondary" type="button" onClick={() => void archiveMedia(item.id)}>ARQUIVAR</button></p></article>)}{!media.length && <div className="empty">Nenhuma mídia neste álbum.</div>}</div>
    </>}
  </section>;
}
