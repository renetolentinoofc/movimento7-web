...
export function AdminSystem() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const [systemResponse, readyResponse] = await Promise.all([
  ...
  return (
    <section className={styles.section} aria-labelledby="system-title">
      ...
      {system ? <div className={styles.grid}>
        <article className={styles.card}><h2>Aplicação</h2><dl><div><dt>Versão</dt><dd>{system.app_version}</dd></div><div><dt>Commit</dt><dd>{system.git_commit}</dd></div><div><dt>Ambiente</dt><dd>{system.environment}</dd></div><div><dt>Python</dt><dd>{system.python}</dd></div></dl></article>
        <article className={styles.card}><h2>Dependências</h2><dl><div><dt>Banco</dt><dd data-good={system.database === "connected"}>{system.database}</dd></div><div><dt>Prontidão</dt><dd data-good={ready?.status === "ready"}>{ready?.status ?? "indisponível"}</dd></div><div><dt>Drive</dt><dd>{system.drive}</dd></div><div><dt>Pagamentos</dt><dd>{system.payment_provider}</dd></div></dl></article>
